...
__all__ = ['ResNetBackbone', 'Blocks', 'BasicBlock', 'BottleNeck']
...
class ResNetBackbone(nn.Layer):
    ...
    def __init__(self,
        ...
        self.channels = self._out_channels
    ...
    def init_weights(self, backbone_path):
        state_dict = paddle.load(backbone_path) # 660
        from collections import OrderedDict
        tmp = OrderedDict()
        for k in state_dict.keys():
            kt = k.replace("backbone.","")
            tmp[kt] = state_dict[k]
        self.set_state_dict(tmp)
...
def construct_backbone(cfg):
    ...
    for k in ResNet_cfg:
        if ResNet_cfg[k] == cfg.args[0]:
            backbone = cfg.type(k)
            break
      # cfg.type: ResNetBackbone   cfg.args: ([3, 4, 23, 3],)
