...
class ResNetBackbone(nn.Layer):
    ...
    def __init__(self, layers, dcn_layers=[0, 0, 0, 0], dcn_interval=1, atrous_layers=[], block=Bottleneck, norm_layer=nn.BatchNorm2D):
        ...
        self.backbone_modules = [m for m in self.children() if isinstance(m, nn.Conv2D)]
...
class VGGBackbone(nn.Layer):
    ...
    def __init__(self, cfg, extra_args=[], norm_layers=[]):
        ...
        self.norms = nn.LayerList([nn.BatchNorm2D(self.channels[l]) for l in norm_layers])
...
def construct_backbone(cfg):
    ...
    backbone = cfg.type(*cfg.args)  # cfg.type: ResNetBackbone   cfg.args: ([3, 4, 23, 3],)
