...
def postprocess(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
    ...
    if score_threshold > 0:
        ...
        for k in dets:
            if k != 'proto':
                dets[k] = paddle.gather(dets[k],paddle.nonzero(keep).flatten(),axis=0)
                #dets[k] = dets[k][keep]
    ...
    if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch:
        ...
        masks = paddle.mm(proto_data,masks.t())
        ...
        masks = masks.transpose((2, 0, 1))#.contiguous()

        if cfg.use_maskiou: # False
            with timer.env('maskiou_net'):                
                with paddle.no_grad():
                    maskiou_p = net.maskiou_net(masks.unsqueeze(1))
                    maskiou_p = paddle.gather(maskiou_p, dim=1, index=classes.unsqueeze(1)).squeeze(1)
                    if cfg.rescore_mask:
                        if cfg.rescore_bbox:
    ...
    boxes = boxes.astype('int64')#.long()

    if cfg.mask_type == mask_type.direct and cfg.eval_mask_branch: # False
        # Upscale masks
        full_masks = paddle.zeros(masks.shape[0], h, w)
...
def display_lincomb(proto_data, masks):
    ...
    for kdx in range(1):
        ...
        test = paddle.sum(proto_data, -1).cpu().numpy()
        ...
        for y in range(arr_h):
            for x in range(arr_w):
                ...
                arr_img[y*proto_h:(y+1)*proto_h, x*proto_w:(x+1)*proto_w] = (proto_data[:, :, idx[i]] / paddle.max(proto_data[:, :, idx[i]])).cpu().numpy() * coeffs_sort[i]
