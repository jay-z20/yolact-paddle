...
class SavePath:
    ...
    def get_path(self, root:str=''):
        file_name = self.model_name + '_' + str(self.epoch) + '_' + str(self.iteration) + '.dpparams'
    ...
    def from_str(path:str):
        ...
        if file_name.endswith('.dpparams'):
...
def make_net(in_channels, conf, include_last_relu=True):
    ...
    def make_layer(layer_cfg):
        ...
        if isinstance(layer_cfg[0], str):
            ...
            if kernel_size > 0:
                layer = nn.Conv2D(in_channels, num_channels, kernel_size, weight_attr=ParamAttr(initializer=XavierUniform()),bias_attr=ParamAttr(initializer=Constant(0.)), **layer_cfg[2])
            else:
                if num_channels is None:
                    layer = InterpolateModule(scale_factor=-kernel_size, mode='bilinear', align_corners=False, **layer_cfg[2])
                else:
                    layer = nn.Conv2DTranspose(in_channels, num_channels, -kernel_size, weight_attr=ParamAttr(initializer=XavierUniform()),bias_attr=ParamAttr(initializer=Constant(0.)), **layer_cfg[2])
