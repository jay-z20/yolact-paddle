...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if undo_transform:
        ...
        img_gpu = paddle.to_tensor(img_numpy).cuda()
    else:
        img_gpu = img / 255.0
        h, w, _ = img.shape
    ...
    with timer.env('Copy'):
        #tk = min(args.top_k,t[1].shape[0])
        #scores, idx = t[1].topk(axis=0,k=tk,largest=True, sorted=True)
        #t[1].topk(axis=0,k=2,largest=True, sorted=True)[1]
    ...
    def get_color(j, on_gpu=None):
        ...
        if on_gpu is not None and color_idx in color_cache[on_gpu]:
            ...
            color = paddle.to_tensor(color,dtype="float32") / 255.
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        masks = masks[:num_dets_to_consider, :, :].unsqueeze(-1)
        # masks = masks[:num_dets_to_consider, :, :, None]
        
        # Prepare the RGB images for each mask given their color (size [num_dets, h, w, 1])
        colors = paddle.concat([get_color(j).reshape((1, 1, 1, 3)) for j in range(num_dets_to_consider)], axis=0)
        masks_color = masks.tile((1, 1, 1, 3)) * colors * mask_alpha
    ...
    img_numpy = paddle.cast(img_gpu * 255,"uint8").cpu().numpy()
...
def prep_metrics(ap_data, dets, img, gt, gt_masks, h, w, num_crowd, image_id, detections:Detections=None):
    ...
    with timer.env('Postprocess'):
        ...
        try:
            if len(classes)==0 or (classes is None) or classes.shape[0] == 0:
                return
        except:
            return
    ...
    if args.output_coco_json:
        with timer.env('JSON Output'):
            boxes = boxes.cpu().numpy()
            masks = masks.reshape((-1, h, w)).cpu().numpy()
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    #im = cv2.resize(im,(cfg.max_size,cfg.max_size),interpolation=cv2.INTER_LINEAR )
    frame = paddle.to_tensor(im,dtype='float32')#.float()  #.cuda().float()
    batch = FastBaseTransform()(frame.unsqueeze(0))
    preds = net(batch) # [] "detection": box mask class score proto
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    transform = FastBaseTransform() #torch.nn.DataParallel(FastBaseTransform()).cuda()
    ...
    def transform_frame(frames):
        with paddle.no_grad():
            frames = [paddle.to_tensor(frame,dtype="float32") for frame in frames] # 
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    print("dataset_size",dataset_size,len(dataset_indices))
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                ...
                #print("image_idx",image_idx,dataset.ids[image_idx])
...
if __name__ == '__main__':
    ...
    with paddle.no_grad():
        ...
        if args.image is None and args.video is None and args.images is None:
            print("valid_images",cfg.dataset.valid_images,cfg.dataset.valid_info)
